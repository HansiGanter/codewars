...
def parse_int(string):
    ...
    if 'thousand' in string:
        return get_hundred((string.split('thousand')[0])+'thousand') + get_hundred(string.split('thousand')[1])
    else:
        return get_hundred(string)


def get_hundred(string_numbers):
    result = 0
    for x in string_numbers.split(' '):
        if x in numbers:
            result += numbers[x]
        if x in multipliers:
